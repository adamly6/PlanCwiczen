...
import kotlinx.android.synthetic.main.niedziela_add.*

class NiedzielaAddActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.niedziela_add)
    ...
    class AddItem(val nazwa_cwiczenie: CwiczenieAdd, val applicationContext: Context) : Item<GroupieViewHolder>() {
        ...
        override fun bind(viewHolder: GroupieViewHolder, position: Int) {
            ...
            viewHolder.itemView.cwiczenie_add.setOnClickListener {
                ...
                val ref1 = FirebaseDatabase.getInstance().getReference("/users/$uid/cwiczenia/niedziela")
                var licznik = 1
                ref1.addListenerForSingleValueEvent(object : ValueEventListener {
                    ...
                    override fun onDataChange(snapshot: DataSnapshot) {
                        ...
                        val ref = FirebaseDatabase.getInstance().getReference("/users/$uid/cwiczenia/niedziela/${nazwa_cwiczenie.id}")
